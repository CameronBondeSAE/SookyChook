...
namespace Rob
{
    public class FollowPath : MonoBehaviour
    {
        ...
        public void TakePath()
        {
	        ...
	        if (pathFinding != null && pathFinding.path.Count > 0 && !tassieDevilModel.atTarget)
            {
                wander.enabled = false;
                forward.speed  = newSpeed;
                Vector3 targetNodeWorld     = pathFinding.ConvertGridToWorldSpace(targetPathNode.gridPos);
