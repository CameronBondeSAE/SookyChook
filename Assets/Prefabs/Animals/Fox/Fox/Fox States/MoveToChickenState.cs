...
namespace Aaron
{
    public class MoveToChickenState : AntAIState
    {
        ...
        public override void Execute(float aDeltaTime, float aTimeScale)
        {
            ...
            if (pathfinding != null && Vector3.Distance(owner.transform.position,
                                                        pathfinding.VectorToInt(currentNodeTarget.coords)) < 0.5f)
            {
                currentNodeIndex++;
                currentNodeTarget = pathfinding.path[currentNodeIndex];
